...
typedef enum
{
  ...
  MCI_COMMAND_EXECUTED_SUCCESSFULLY,  /*!< If the buffered command has been
                                            executed successfully.*/
  MCI_COMMAND_EXECUTED_UNSUCCESSFULLY /*!< If the buffered command has been
...
typedef enum
{
  ICLWAIT = 12,         /*!< Persistent state, the system is waiting for ICL
                           ...
                           is moved to IDLE */
  IDLE = 0,             /*!< Persistent state, following state can be START
                           if a start motor command has been given or
                           ALIGNMENT if a start alignment command has been
                           given */
